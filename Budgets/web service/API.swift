...
class API: NSObject {
    ...
    class func facebookLogin(email: String,password: String, completion: @escaping (_ error : Error?, _ success: Bool)->Void) {
        let url = "http://bare3.com.sa/budgets/api/loginUser"
    ...
    class   func register (  name : String, email: String, password : String, completion: @escaping (_ error : Error?, _ success: Bool,_ state :Bool)->Void) {
                    let url = "http://bare3.com.sa/budgets/api/registerUser"
                        ...
                        .responseJSON { responce in
                            ...
                            case.success(let value):
                                ...
                                class   func forgetPassword (   email: String, completion: @escaping (_ error : Error?, _ success: Bool)->Void) {
                                    let url = "http://bare3.com.sa/budgets/api/MessageResetPasswordSent"
                                    let parameters = [
                                                      "email" : email
                                                      ]
    ...
    class func getBudgets (completion: @escaping (_ error : Error?, _ budgets: [Budget])->Void) {
        guard let email = helper.getEmail() else {
            completion ( nil , [] )
            return
        }
        
        let url = "http://bare3.com.sa/budgets/api/all_budgets"
        let parameters = ["email" :  email]
        Alamofire.request(url, method: .get, parameters: parameters, encoding: URLEncoding.default, headers: nil)
            .validate(statusCode: 200..<900)
            .responseJSON { responce in
                ...
                case.success(let value):
                    ...
                    for data in dataArr {
                        ...
                        let budget = Budget()
                        budget.b_amount = data["b_amount"]?.int ?? 0
                        budget.b_name = data["b_name"]?.string ?? ""
                        budget.created_at = data["created_at"]?.string ?? ""
                        
                        budgets.append(budget)
                        
                    }
                    completion(nil, budgets)
    ...
    class func getUserData (completion: @escaping (_ error : Error?, _ users: [userData])->Void) {
        ...
        let url = "http://bare3.com.sa/budgets/api/getUserData"
        let parameters = ["email" :  email]
        Alamofire.request(url, method: .get, parameters: parameters, encoding: URLEncoding.default, headers: nil)
            .validate(statusCode: 200..<900)
            .responseJSON { responce in
        ...
        class func updateUserData (newEmailTxt : String,newNameTxt: String, newPasswordTxt: String, completion: @escaping (_ error : Error?, _ users: [userData])->Void) {
            ...
            let url = "http://bare3.com.sa/budgets/api/upateUserData"
            let parameters = ["email" :  email,
