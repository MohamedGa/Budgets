...
class BudgetListViewController: UIViewController , UISearchBarDelegate {
  ...
  //  @IBOutlet weak var searchBar: UISearchBar!
   ...
   @objc private func handleRefresh() {
        API.getBudgets { (error: Error?, budgets:[Budget]?) in
            if let budgets = budgets {
               self.budgets = budgets
              
                self.data = self.budgets
...
extension BudgetListViewController:  UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.name.text =  data![indexPath.row].b_name
        let bugdetAmount = "\( data![indexPath.row].b_amount)"
        cell.budgetAmount.text = bugdetAmount
        cell.createdAt.text = data![indexPath.row].created_at
    ...
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        ...
        data = searchText.isEmpty ? budgets : data!.filter({(budget: Budget) -> Bool in
            // If dataItem matches the searchText, return true to include it
            return budget.b_name.range(of: searchText, options: .caseInsensitive) != nil
        })
